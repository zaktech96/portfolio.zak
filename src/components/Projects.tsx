...
const TechIcon = ({ name }: { name: string }) => {
  switch (name) {
    ...
    case "Nextjs":
      return (
        <svg
          className="w-3 h-3 sm:w-4 sm:h-4 flex-shrink-0 dark:text-white text-black"
          viewBox="0 0 24 24"
        >
          <path
            fill="currentColor"
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-4 pt-4">
                      {project.github && (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center justify-center gap-2 px-3 sm:px-4 py-2 bg-primary/10 hover:bg-primary/20 border border-primary/20 hover:border-primary/40 rounded-lg text-xs sm:text-sm font-medium transition-all duration-300"
                          aria-label="View Source Code"
                        >
                          <svg
